...
data_compensation <- data.frame(
  Persona = c("Full Sample",
              ...
              "The Middle-Income Resident",
              "The Environmentally Indifferent"),
  WTP_Euro = c(3.90, 7.63, 12.01, 10.11, 8.91, 16.98),
  WTP_T_Ratio = c(-2.05, -2.7, -2.64, -2.27, -2.07, -2.84)
)

# Display the created data frame
print(data_compensation)
...
formattable(data_compensation, 
            align = c("l", rep("r", NCOL(data_compensation) - 1)),
            list(
              `WTP_Euro` = formatter("span", 
                                     style = ~ style(
                                       color = ifelse(`WTP_Euro` > 3.9, "green", 
                                                      ifelse(`WTP_Euro` < 3.9, "red", "black"))
                                     ),
                                     ~ icontext(sapply(`WTP_Euro`, 
                                                       function(x) if (x < 3.9) "arrow-down" else if (x > 3.9) "arrow-up" else ""),
                                                `WTP_Euro`)
              ),
              `Persona` = formatter("span", 
                                    style = ~ style(
            ...
            caption = "Willingness to Pay for better compensation"
)
...
# Reorder the levels of Persona based on WTP_Euro
data_compensation$Persona <- reorder(data_compensation$Persona, -data_compensation$WTP_Euro)

ggplot(data_compensation, aes(x = Persona, y = WTP_Euro, fill = Persona)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = sprintf("%.2f", WTP_Euro)), 
            vjust = -0.5, 
            position = position_dodge(width = 0.9), 
