...
```
ENTIRE POPULATION
1 age_front and p_mob_time positively correlated: younger people are out longer

2 cars per household and and use of pt negatively correlated

3 p_mob_time positively correlated with p_mob_rural


RURAL POPULATION
1 p_central_nonwien and p_place_inhab negatively correlated, shows that smaller places are more likely to be less connected
2 p_central_nonwien negative cor with p_mob_rural meaning people with bad connection to PT are more often in rural areas DUH
3 p_central_nonwien pos cor with p_mob_ptintensity meaning that people who are not well connected, use PT less
```{r}
#ggplot(data = database) + 
  #geom_point(mapping = aes(x = age_front, y = p_mob_time)) +
  #geom_smooth(mapping = aes(x = age_front, y = p_mob_time)) +
#ggtitle ("Age and out how long")
```

##Regression car ownership
```{r}
library(pROC)

# Logistic regression model
database_factor <- database %>%
  mutate(sex_front = as.factor(sex_front),
         p_bundesland_front = as.factor(p_bundesland_front),
         p_educ_front = as.factor(p_educ_front),
...
#sapply(lapply(database_factor, unique), length)

#Regression model
carownership_model <- glm(p_mtools_1 ~ #sex_front +
         #p_bundesland_front +
         p_educ_front +
         #age_front +
         m_known +
         ...
         #kilometrage_per_car_1 +
         kilometrage_per_car_2, 
         family = "binomial", data = database_factor)
...
```
influencing yes i own a car
- apprenticeship significant
- not knowing about MG
- not using public transport (p_mob_ptintensity)