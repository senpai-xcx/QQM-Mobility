...
persona_data_mg <- data.frame(
  Persona_mg = c('The Peripheral Villager', 'The Elderly', 'The Student', 'The Parent', 'The Commuter', 'The Late Night Driver','The Low Income','The Blue Collar Worker','The Climate Change Skeptic','The Season Ticket Holder','Women','Men'),
   MG_Yes = sapply(list(peripheral, elderly, student, parent, commuter, latenight, lowincome, bluecollar, ccskeptic, ticket, women, men), function(persona_mg) sum(persona_mg$choice_diff_yes == 1)),
 MG_No = sapply(list(peripheral, elderly, student, parent, commuter, latenight, lowincome, bluecollar, ccskeptic, ticket, women, men), function(persona_mg) sum(persona_mg$choice_diff_yes == 2))
)
...
ggplot(persona_data_mg, aes(x = Persona_mg)) +
  geom_bar(aes(y = MG_Yes, fill = "Yes"), stat = "identity", position = "dodge") +
  geom_bar(aes(y = -MG_No, fill = "No"), stat = "identity", position = "dodge") +
  coord_flip() +
  labs(title = "Car Ownership by Persona",
       x = "Persona",
       y = "Count") +
  scale_fill_manual(values = c("Yes" = "pink2", "No" = "blue2"),
